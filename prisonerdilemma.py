...
class Simulation():
    ...
    def generate_percentage_plot(self):
        ...
        for i in self.geneticpercentage:
            ...
            with plt.style.context('seaborn-darkgrid'):
                plt.plot(time,self.geneticpercentage[i],'-o',label=self.reverse_lookup[i])
    ...
    def generate_number_plot(self):
        ...
        for i in self.geneticcount:
            ...
            with plt.style.context('seaborn-darkgrid'):
                plt.plot(time,self.geneticcount[i],'-o',label=self.reverse_lookup[i],)
        plt.xlabel("Epoch")
        plt.ylabel("Agents")
    ...
    def plot_percentage_count(self):
        ...
        for i in self.geneticcount:
            print(self.geneticcount[i])
            ax1.plot(time,self.geneticcount[i],'-o',label=self.reverse_lookup[i],)
        ax1.set(xlabel = "Epoch", ylabel = "Population")
        ax1.legend()
        for i in self.geneticpercentage:
            print(self.geneticpercentage[i])
            ax2.plot(time,self.geneticpercentage[i],'-o',label=self.reverse_lookup[i])
